...
def get_emote_image(ctx, emoji: discord.PartialEmoji):
    if emoji.animated:
        ...
        request_file = requests.get(parsed_url)
        open(resources_location + "/emotes_grabbed/temp_emote.png", 'wb').write(request_file.content)
        return discord.File(resources_location + "/emotes_grabbed/temp_emote.png")


def cleanup_emote():
    for files_found in glob.glob(resources_location + "/emotes_grabbed/*"):
